...
var MemStorage = class {
  ...
  async createUpload(insertUpload) {
    ...
    this.uploads.set(id, upload2);
    return upload2;
  }
  async getUpload(id) {
    return this.uploads.get(id);
  }
  async getUploadByFilename(filename) {
    return Array.from(this.uploads.values()).find(
      (upload2) => upload2.filename === filename
    );
  }
};
...
var insertUploadSchema = createInsertSchema(uploads).pick({
  filename: true,
  originalName: true,
  mimetype: true,
  size: true
...
async function registerRoutes(app2) {
  app2.post("/api/upload", upload.single("file"), async (req, res) => {
    try {
      ...
      res.json({
        ...
        url: `/api/uploads/${upload2.filename}`
      });
    } catch (error) {
  ...
  app2.get("/api/uploads/:filename", async (req, res) => {
    try {
      ...
      const upload2 = await storage.getUploadByFilename(filename);
      if (!upload2) {
      ...
      res.setHeader("Content-Type", upload2.mimetype);
